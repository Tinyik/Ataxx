...
class Move {
    ...
    boolean isExtend() {
        int colOffset = abs(_col1 - _col0);
        int rowOffset = abs(_row1 - _row0);
        boolean identity = (_col1 == _col0) && (_row1 == _row0);
        return colOffset <= 1 && rowOffset <= 1 && !identity;
    }
    ...
    boolean isJump() {
        if (isExtend()) {
            ...
            int rowOffset = abs(_row1 - _row0);
            boolean identity = (_col1 == _col0) && (_row1 == _row0);
            return colOffset <= 2 && rowOffset <= 2 && !identity;
        }
    }
